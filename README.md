...
Recall is a mobile-first multilingual memory and transcription prototype. Phase 0 is a functionality spike: it validates a continuous local recording pipeline, live Gemini transcription, timestamped bookmarks, reconnects, and session rotation without building the wider product.
...
The expected response is `{"ok":true}`. `POST /token` mints a single-use ephemeral token constrained to the transcribe-live model and text transcription configuration.
...
- `server/index.ts` — local Node/TypeScript ephemeral-token server.
...
These validate TypeScript, server TypeScript, pure session/transcript/bookmark behavior, and ESLint. They do not validate microphone hardware, native permissions, codec output, the network route from a device to the token server, or live Gemini account/model access.
...
Validate the pipeline on a physical Android device and an iOS simulator/device, including permission denial, server-offline recording, reconnect, a forced rotation with a shortened test threshold, and inspection of the saved local file. Then harden the native audio lifecycle and token endpoint before adding local session persistence or product features.
