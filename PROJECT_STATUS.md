...
- Added a language-agnostic audio-grounded transcript repair provider using `gemini-3.5-flash-lite`. D2 receives the original shared audio and successful A transcript only; it never uses C2 output.
- Added optional advisory `TranscriptLanguageContext` with automatic/unknown defaults and a benchmark-only hinted mode for the current difficult multilingual sample. The generic repair prompt contains no Moroccan, Darija, country, or script-specific requirement.
...
- Added product session transcript layers: live finalized transcript, raw A transcript, repaired D2 transcript, and an explicit preferred fallback selection (D2 > A > live). This is a replaceable default preference, not a ground-truth claim.
...
mobile saved WAV → server /benchmark → temporary local WAV
                  ...
                  ├── same Gemini File + successful A text → gemini-3.5-flash-lite audio-grounded repair (D2)
                  └── same Gemini File + A/B/C candidates → optional reconciliation (D)
```

Recall's intended product direction is worldwide multilingual capture and memory. Users may speak one language, several languages, regional dialects, or code-switched speech. Darija is the current difficult multilingual benchmark because it exposes weaknesses in generic ASR; it is not a product-language requirement. Language-specific behavior belongs in provider capabilities, optional advisory session context, vocabulary/context seams, and benchmarks rather than country-specific core logic.
...
23. Session deletion removes database metadata first, then only the target session audio directory. If filesystem cleanup fails, the app reports the recoverable cleanup issue instead of deleting unrelated files.
24. SessionProvider now uses one stable repository initialization promise and exposes an explicit warm-up/error boundary. Loading and SQLite initialization failures render a Recall-styled recovery screen with retry; the app does not leave a featureless screen after JavaScript has loaded.
25. Session detail passes `null` to the SDK 57 audio player until a saved URI is available. Missing audio is handled as a recoverable playback/seek error instead of being allowed to initialize an invalid source.
26. Live capture diagnostics are DEV-only. They report token/socket/setup milestones, sent/dropped chunks, server/interim/final counts, close code, and safe errors; they never include credentials, audio, or transcript contents.
...
- Phase 1 static result — `npm run typecheck`, `npm run server:check`, `npm run lint`, and `npm test` passed with 16 test files and 88 tests. Repository tests cover schema initialization/versioning, create/list/reload, rename/delete/bookmark persistence, processing failure preservation, and D2 > A > live default preference selection.
- Phase 1.1 transcript semantics — D2 can improve difficult speech but can also worsen already-correct wording or normalize it. Recall now preserves live, raw-final, and repaired layers independently and calls the selected layer preferred rather than authoritative. The current repaired > raw-final > live order is documented as a replaceable default preference, not guaranteed truth.
...
- Android emulator local-session sanity — microphone permission was granted, a non-zero 2,694,700-byte WAV was finalized, Gemini Live reached token/socket/setup-ready milestones, and the local session was persisted. After an emulator force-stop/relaunch with a healthy Metro URL, Home listed the session again; detail loaded offline, Media3 playback advanced, and the emulator-only session was renamed successfully. No human-speech recognition result was claimed from this silent emulator capture.
- Android emulator launch caveat — a plain `monkey -p` launch on a fresh development client can open the dev-client picker or an empty native surface when no Metro project URL is selected. This is a development-client/Metro launch condition; selecting the healthy LAN project or using the documented reversed-localhost deep link loads Recall normally. No JS, SQLite, or native crash was observed in the healthy-bundle run.
- Phase 1 playback-navigation regression — on the physical `SM_X516B` tablet, leaving an existing session through the HISTORY action reproduced `ERR_USING_RELEASED_SHARED_OBJECT`. The first relevant React log was `Call to function 'AudioPlayer.pause' has been rejected` / `Cannot use shared object that was already released`, with the stack pointing to the session detail unmount cleanup.
- Phase 1 playback lifecycle fix — SDK 57 `useAudioPlayer` is hook-owned and automatically releases its `AudioPlayer` on component unmount. The session route no longer calls `player.pause()` from unmount cleanup; mounted play/pause, timeline seek, bookmark seek, and delete/navigation behavior remain separate from disposal.
- Phase 1 playback fix physical result — after reloading the changed JavaScript through the existing Metro/dev client (no native rebuild), the physical tablet passed session detail → HISTORY, detail → Android Back, playing detail → Android Back, and paused detail → Android Back. Home rendered after each path, and no new React/AudioPlayer released-object error appeared in filtered logcat.
- Phase 1 delete lifecycle result — disposable sessions were deleted through the normal confirmation flow on both the emulator and the physical tablet. On the tablet, Home went from two rows to the original saved row after deleting the newer disposable recording; after force-stop/relaunch, only the original row remained and its app-owned audio file remained. The physical saved session was not deleted or modified.
